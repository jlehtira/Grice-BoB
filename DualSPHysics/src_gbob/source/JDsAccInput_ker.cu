...
__global__ void KerAddAccInputLin(unsigned n,unsigned pini,typecode codesel1,typecode codesel2
  ,float3 gravity,bool setgravity,double3 acclin,const typecode *code,float3 *ace
    /*JL*/,const double2 *posxy,const double *posz,const float4 *velrhop/*JL*/)
{
  const unsigned pp=blockIdx.x*blockDim.x + threadIdx.x;
  if(pp<n){
    const unsigned p=pp+pini;
    ...
    if(codesel1<=tav && tav<=codesel2){
      ...
      double rho_a = 1.2;        // kg/m³
      double C_a10 = 2*1.2E-3;   //
      double theta_a = 25*3.14159/180.0;
      double rho_w = 1000;       // kg/m³
      double C_w = 5E-3;
      double theta_w = 25*3.14159/180.0;

      double du = acclin.x - velrhop[p].x;
      double dv = acclin.z - velrhop[p].z;


      double tau_a_x = rho_a * C_a10   *   (abs(du) * cos(theta_a) * du  +  abs(dv) * sin(theta_a) * dv);
      double tau_a_z = rho_a * C_a10   *   (abs(dv) * cos(theta_a) * dv  -  abs(du) * sin(theta_a) * du);

      double tau_w_x = rho_w * C_w   *   (abs(velrhop[p].x) * cos(theta_w) * (0-velrhop[p].x)  +  abs(velrhop[p].z) * sin(theta_w) * (0-velrhop[p].z));
      double tau_w_z = rho_w * C_w   *   (abs(velrhop[p].z) * cos(theta_w) * (0-velrhop[p].z)  -  abs(velrhop[p].x) * sin(theta_w) * (0-velrhop[p].x));


      accx += (tau_a_x + tau_w_x) / velrhop[p].w;
      accz += (tau_a_z + tau_w_z) / velrhop[p].w;

      //-Adds linear acceleration.
      // JL test wind factor of 2.5 % applied to wind speed
//      accx += 0.025 * acclin.x /*JL*/ - velrhop[p].x;
////JL//      accy+=acclin.y /*JL*/ - velrhop[p].y;
//      accz += 0.025 * acclin.z /*JL*/ - velrhop[p].z;
      //-Subtract global gravity from the acceleration if it is set in the input file.
      if(!setgravity){
        accx-=gravity.x;  accy-=gravity.y;  accz-=gravity.z; 
      }
  ...
  ,tfloat3 gravity,const typecode *code,const double2 *posxy,const double *posz,const float4 *velrhop,float3 *ace,cudaStream_t stm)
{
  if(n){
    ...
    else          KerAddAccInputLin <<<sgrid,SPHBSIZE,0,stm>>> (n,pini,codesel1,codesel2,Float3(gravity),setgravity,Double3(acclin),code,ace /*JL*/,posxy,posz,velrhop/*JL*/ );
  }
}
